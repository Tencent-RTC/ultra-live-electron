<template>
  <div class="tui-audience-connection">
    <LiveChildHeader :title="t('Voice Chat Management')"></LiveChildHeader>
    <div class="tui-audience-connection-setting">
      <div class="tui-audience-connection-layout">
        <TUIButton class="tui-audience-connection-layout-grid" :class="currentLayout === TUIStreamLayoutMode.Grid ? 'selected' : ''"  @click="handleLayoutChange(TUIStreamLayoutMode.Grid)">
          {{ t('Grid Layout') }}
        </TUIButton>
        <TUIButton class="tui-audience-connection-layout-float" :class="currentLayout === TUIStreamLayoutMode.Float ? 'selected' : ''" @click="handleLayoutChange(TUIStreamLayoutMode.Float)">
          {{ t('Float Layout') }}
        </TUIButton>
      </div>
      <div class="tui-audience-connection-adjusting">
    ...
    <div class="tui-audience-connection-list-container" >
      <div class="tui-audience-connection-list" >
        <div class="tui-audience-connection-list-title">
          <span>{{ t('Apply for chat') }}</span>
          <span v-if="isAllowed">{{applyNumber}}</span>
        </div>
        <div class="tui-audience-connection-member">
          <span v-if="!isAllowed" class="tui-audience-connection-status">{{ t('Not allowed') }}</span>
          <div v-else v-for="user in applyOnSeatList" :key="user.userId"  class="tui-audience-connection-apply">
            <img class="tui-audience-connection-avatar" :src="user.avatarUrl" alt="">
            <span class="tui-audience-connection-name">{{ user.userName || user.userId }}</span>
            <div class="tui-audience-connection-options">
              <span class="tui-audience-connection-accept" @click="handleUserApply(user, true)">{{ t('Accept')}}</span>
              <span class="tui-audience-connection-reject" @click="handleUserApply(user, false)">{{ t('Rejection')}}</span>
            </div>
          </div>
        </div>
      </div>
      <div class="tui-audience-connection-list">
        ...
        <div class="tui-audience-connection-member">
          <span class="tui-audience-connection-status" v-if="!isAllowed">{{ t('Not allowed') }}</span>
          <div v-else class="tui-audience-connection-seat" v-for="( item, index ) in validAnchorList" :key="item.userInfo.userId || index">
            <span class="tui-audience-connection-seatIndex">{{item.seat}}</span>
            <img v-if="item.userInfo.avatarUrl" class="tui-audience-connection-avatar" :src="item.userInfo.avatarUrl" alt="">
            <span class="tui-audience-connection-name"> {{ item.userInfo.userName || item.userInfo.userId }}</span>
            <mic-more-icon class="tui-audience-connection-more"  @click.stop="handleShowMemberControl(item)" ></mic-more-icon>
...
const { applyOnSeatList, seatedList } = storeToRefs(currentSourceStore);
const { t } = useI18n();
...
const applyNumber = computed(() => {
  return '(' + applyOnSeatList.value.length + ')'
});
...
watch(seatedList, (newVal,val) => {
  logger.debug(`${logPrefix}watch seatedList:`, newVal, val);
  if (newVal) {
    if (newVal.length > 8) {
      logger.warn(`${logPrefix}watch currentAnchorList: too many anchor, max 8, current: ${newVal.length}`);
    }
    for (let i = 0; i < 8; i++) {
      if (i < newVal.length) {
...
.tui-audience-connection{
  ...
  .tui-audience-connection-setting {
    font-size: $font-live-connection-layout-text-size;
    background-color: var(--bg-color-dialog);
  }

  .tui-audience-connection-layout {
    display: flex;
    flex-direction: row;
    justify-content: space-around;
    padding-top: 0.5rem;

    .tui-audience-connection-layout-grid,
    .tui-audience-connection-layout-float {
      flex: 0 0 16rem;
      height: 3rem;
      line-height: 3rem;
      border-radius: 0.5rem;
  ...
  .tui-audience-connection-adjusting {
    color: var(--text-color-primary);
